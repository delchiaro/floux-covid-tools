...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, anchors, annotations):
        ...
        for j in range(batch_size):
            ...
            try:
                alpha_factor = torch.ones(targets.shape).to(classification.device) * alpha
            except:
                print(targets)
                print(targets.shape)
            ...
            cls_loss = torch.where(torch.ne(targets, -1.0), cls_loss, torch.zeros(cls_loss.shape).to(classification.device))
...
class LevelAttention_loss(nn.Module):
    ...
    def forward(self, img_batch_shape, attention_mask, bboxs):
        ...
        for j in range(batch_size):
            ...
            if bbox_annotation.shape[0] == 0:
                mask_losses.append(torch.tensor(0).float().to(attention_mask.device))
                continue
            ...
            for id in range(len(attention_mask)):
                ...
                mask_gt = mask_gt.to(attention_map.device)
